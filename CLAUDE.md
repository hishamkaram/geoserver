...
- **v2 lives at module path `github.com/hishamkaram/geoserver/v2`** under the `/v2/` subdirectory (its own `go.mod`). Latest published tag is `v2.0.0-beta.1` — public API is **frozen for review** as of beta.1; breaking changes will not land in subsequent betas without a strong reason. v2 has full v1 feature parity at `master` plus surfaces v1 never had — per-service OWS settings, file-upload publishing on stores, layer–style associations, GeoWebCache (cache config + seed/truncate + diskquota), the Importer extension, the full ACL surface (`Layers`/`Services`/`REST`/`Catalog`), the Resource API (data-dir byte-stream access), and the OWS read-only trio (GetCapabilities + DescribeFeatureType + DescribeCoverage). Run `make test-v2-unit` for v2 unit tests and `make test-v2-integration` for integration tests against the compose stack. v2 fixes the v1 wounds that cannot be healed without breaking changes (exported mutable fields, four-positional-arg `PublishPostgisLayer`, etc.).
